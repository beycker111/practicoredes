		...
		try {
			...
			if(headerLine!=null)
			{
				...
				if(httpMethod.equals("GET"))
				{
					...
					if(httpQueryString.contains("/?usuario="))
					{
						System.out.println("Get method processed");
						String[] response =  httpQueryString.split("usuario=");
						String[] response2 = httpQueryString.split("pass=");
						StringBuilder responseBuffer =  new StringBuilder();
						responseBuffer
						
						//response 1 contiene el usuario
						//response2 contiene la contrasena
						
						//Hay que buscarlos entre los jugadores existentes
